...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
            <a href="{{ route('teacher.subjects.index') }}" class="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-indigo-600 transition-colors mb-2">
                <i class='bx bx-arrow-back'></i>
                Back to Subjects
            </a>
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center text-white font-bold text-xl">
        ...
        <form method="POST" action="{{ route('teacher.subjects.destroy', $subject) }}" onsubmit="return confirm('Delete this subject and all its data? This action cannot be undone.')">
            @csrf @method('DELETE')
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        <div class="p-5">
            <form method="POST" action="{{ route('teacher.sections.store', $subject) }}" class="flex gap-3 mb-6">
                ...
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl font-semibold transition-all shadow-lg shadow-indigo-500/25">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="border border-slate-200 rounded-xl p-5 hover:border-indigo-200 hover:shadow-lg hover:shadow-indigo-500/5 transition-all duration-200">
                    ...
                    <div class="space-y-2 mb-4 max-h-40 overflow-y-auto">
                        ...
                        <div class="flex items-center justify-between py-2 px-3 rounded-lg bg-slate-50">
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-xs font-bold">
                                    {{ strtoupper(substr($student->name, 0, 1)) }}
                                </div>
                                <span class="text-sm text-slate-700">{{ $student->name }}</span>
                                <span class="text-xs text-slate-400">({{ $student->ic }})</span>
                            </div>
                            <form method="POST" action="{{ route('teacher.sections.removeStudent', [$subject, $section, $student]) }}">
                                @csrf @method('DELETE')
                                <button class="w-6 h-6 rounded-full hover:bg-red-100 flex items-center justify-center text-slate-400 hover:text-red-500 transition-all">
                    ...
                    <form method="POST" action="{{ route('teacher.sections.addStudent', [$subject, $section]) }}" class="flex gap-2">
                        @csrf
                        <select name="student_id" required class="flex-1 text-sm border border-slate-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
